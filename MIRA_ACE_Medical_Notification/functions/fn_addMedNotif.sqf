...
LOG(format["Custom Message is: %1", GVAR(Message)]);
_id = ["ace_medicalMenuOpened", {
		params["_player", "_target", ""];
		if(!GVAR(Enabled)) exitWith {};
		if(_player == _target && !GVAR(ShowToSelf)) exitWith {};
		if(time - GVAR(lastMessageTime) < MAX_RATE_SECONDS) exitWith {/* Rate Limited */};
